 ...
 * CRIT-02 — POST now requires authentication. Submitter identity is recorded
 *            in the `submittedBy` column so every ticket has an audit trail.
 * HIGH-04  — GET role check updated to allow both 'it_staff' AND 'ADMIN'.
 */
...
/**
 * POST /api/tickets
 * Create a new ticket with AI triage.
 * Requires authentication — anonymous submissions are rejected.
 */
export async function POST(request: NextRequest) {
  try {
    // CRIT-02: require a valid session before touching Groq or the DB
    const session = await getServerSession(authOptions);
    if (!session) {
      return createErrorResponse('Unauthorized', 401);
    }

    // Record who submitted the ticket for the audit trail
    const submittedBy =
    ...
    // Call Groq service for AI triage (input is sanitized inside triageTicket)
    ...
    const [newTicket] = await db
      ...
      .values({
        userInput,
        submittedBy,   // CRIT-02: persist submitter identity
 ...
 * HIGH-04: accepts both 'it_staff' and 'ADMIN' roles.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const urgency = searchParams.get('urgency')
      ? parseInt(searchParams.get('urgency')!, 10)
      : undefined;
    ...
    if (urgency)    conditions.push(eq(tickets.urgencyScore, urgency));
    if (status)     conditions.push(eq(tickets.status, status));
    if (dateFrom)   conditions.push(gte(tickets.createdAt, new Date(dateFrom)));
    if (dateTo)     conditions.push(lte(tickets.createdAt, new Date(dateTo)));

    const orderByColumn =
      sortBy === 'urgencyScore' ? tickets.urgencyScore :
